import cv2
import numpy as np
# Videodaki Çokgenleri tespit etme
...
cv2.createTrackbar("Upper-Hue","Settings",0,255,nothing)
...
while 1:
    ...
    # şeklin merkez noktalarına ihtiyaç var
    for cnt in contours:
        area = cv2.contourArea(cnt)
        epsilon =0.02*cv2.arcLength(cnt,True)
        # İyileştirme yapılacak countrous yaklaşır
        approx = cv2.approxPolyDP(cnt, epsilon, True)

        # ravel ==> bir boyutlu diiye çeviriyor
        # çokgenin ilk değerler
        x = approx.ravel()[0]
        y = approx.ravel()[1]
        ...
        if area > 400:
            ...
            if len(approx) == 3:
                ...
                cv2.putText(frame, "hekzagon", (x,y), font, 1,(0,0,0))
            elif len(approx) > 6:
