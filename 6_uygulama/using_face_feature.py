...
while 1:
    ...
    if len(contours)>0:
        try:
            ...
            cv2.circle(roi,extBotton,5,(0,255,0),2)

            # Daireler birleştirildi
            cv2.line(roi,extLeft,extTop,(255,0,0),2)
            cv2.line(roi,extTop,extRight,(255,0,0),2)
            cv2.line(roi,extRight,extBotton,(255,0,0),2)
            cv2.line(roi,extBotton,extLeft,(255,0,0),2)

            # sağ köşedeki açıyı bulmak için
            a = math.sqrt((extRight[0]-extTop[0])**2+(extRight[1]-extTop[1])**2)
            b = math.sqrt((extBotton[0]-extRight[0])**2+(extBotton[1]-extRight[1])**2)
            c = math.sqrt((extBotton[0]-extTop[0])**2+(extBotton[1]-extTop[1])**2)
            try:
